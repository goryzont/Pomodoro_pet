...
async def test_get_google_redirect__success(auth_service: AuthService, settings: Settings):
    settings_google_redirect_url = settings.google_redirect_url

    auth_service_google_redirect_success = auth_service.get_google_redirect_url()

    assert  settings_google_redirect_url == auth_service_google_redirect_success


async def test_get_yandex_redirect__success(auth_service: AuthService, settings: Settings):
    settings_yandex_redirect_url = settings.yandex_redirect_url

    auth_service_yandex_redirect_success = auth_service.get_yandex_redirect_url()
...
async def test_generate_access_token_success(auth_service: AuthService, settings: Settings):
    user_id = 1
